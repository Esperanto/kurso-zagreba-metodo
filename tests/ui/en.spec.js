...
async function newStandalonePwaContext(browser) {
...
async function readPwaRecord(page, storeName, key) {
...
test('PWA-stato restas malŝaltita en ordinara retumilo', async ({ browser }) => {
  const context = await browser.newContext();
...
test('PWA-stato konservas ekzercajn respondojn, pozicion kaj lastan paĝon', async ({ browser }) => {
  ...
  const pageKey = 'en|page|/en/01/ekzerco1/';
  let page = await context.newPage();
  ...
  const solution = await page.locator('#vico-1-1-1').getAttribute('data-solvo');
  await page.locator('.solvu[data-form-id="1-1"]').click();
  await expect.poll(async () => {
    const record = await readPwaRecord(page, 'inputs', inputKey);
    return record && record.value;
  }).toBe(solution);
